...
Analyse the text in four passes — Pass 0 first, then Passes 1-3.

### Pass 0: Origin diagnostic — run this BEFORE flagging anything

The biggest false-positive in AI detection is mistaking non-native English speakers (ESL) for AI. Both produce surface-level patterns that look similar: simple vocabulary, formal-sounding constructions, low specificity. But the underlying causes differ, and so should the response. Calling an ESL writer's email "AI slop" is both wrong and disrespectful.

Before flagging anything, check the text for these **positive ESL markers** (not absences — actual things that are present in the text):

1. **Textbook business-English openers** — "I am writing to inform you...", "We are pleased to announce...", "Please find attached...", "Looking forward to hear from you" (note the verb-form error in the last one). These are taught in ESL business-English courses worldwide. AI prefers warmer-sounding variants ("Hope this email finds you well", "Just wanted to reach out").
2. **Article irregularities** — Missing or extra "the/a/an" ("the team will discuss in meeting", "I will send you a information"). AI almost never makes article mistakes.
3. **Preposition mismatches** — Subtle non-standard prepositions ("discussion about" where US English expects "of", "in next week" instead of "next week", "explain me" instead of "explain to me"). AI uses textbook collocations.
4. **L1-translation constructions** — Phrases grammatically correct but slightly off-idiom, often direct translations from a first language ("make a photo" for "take a photo", "I will be in disposition" for "I will be available"). AI doesn't translate; it generates.
5. **Verb-tense quirks** — Heavy use of present-perfect-continuous where simple past would be natural ("We have been working on this for two years and are very excited to share it"). AI defaults to simple present and simple past.
6. **Absence of idiomatic flourishes** — AI sprinkles idioms ("at the end of the day", "moving the needle", "hit the ground running"). ESL writers tend to avoid idioms because they're high-risk. *Use this only as supporting evidence, not a primary signal — its absence proves nothing on its own.*

**Decision rule:**

- **2+ positive ESL markers present** → Classify as ESL writing. Cap the slop score at 5 (texts can still be improved, but they're not AI — penalising them as AI is wrong). Reframe the analysis as "naturalising for native readers" not "removing AI tells." In the rewrite, fix the actual grammatical and idiomatic friction while preserving the writer's voice — they wrote this, even if in a second language. Be respectful in tone.

- **0-1 ESL markers AND the text is uniformly polished, abstract, and impersonal** → Proceed as AI. Polished + zero-personality + zero-mistakes is the AI fingerprint. Score normally.

- **Genuinely uncertain** → Ask the user one line before scoring: "Quick check — was this written by you (or another person) in a non-native language, or generated/heavily-edited by AI? It changes how I score it." Then proceed accordingly.

Note your origin assessment explicitly in the output (see Output format below).

Once origin is settled, analyse the text in the remaining three passes:
...
**Origin assessment:** [AI-generated / ESL writing / Native-English formal / Mixed human + AI / Genuinely good human writing] — one sentence explaining the evidence
